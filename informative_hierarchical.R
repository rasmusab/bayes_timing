...
data_list = list(
  # It is here the fixed parameters for the subjective priors are specified
  # The values below are just an example. For this to work you would have to specify
  # priors for all isi levels. For example, below c(30, 60, 90, 120, 150) refers
  # to mean of the mean of the sigma parameter for the isi levels c(600, 1200, 1800, 2400, 3000)
  mean_mean_sigma = c(30, 60, 90, 120, 150),
  sd_mean_sigma = c(24, 48, 72, 96, 120),
  median_sd_sigma =  c(25.8, 51.6, 77.4, 103.2, 129),
  mean_mean_mu = c(-24, -48, -72, -96, -120),
  sd_mean_mu = c(48, 96, 144, 192, 240),
  median_sd_mu = c(42, 84, 126, 168, 210),
...
inits_list$mean_sigma <- apply(inits_list$sigma, 2, mean)
...
model_string <- "model {
  ...
  for(isi_i in 1:n_isi) {
    ...
    # Transformation for the log-normal on sd_sigma
    ...
    sd_sigma[isi_i] ~ dt(0, 1/pow(median_sd_sigma[isi_i], 2), 1) T(0, )
    mean_sigma[isi_i] ~ dlnorm(logmu_mean_sigma[isi_i], 1/pow(logsigma_mean_sigma[isi_i], 2))

    # Transformation for the log-normal on mean_sigma
    logsigma_mean_sigma[isi_i] <- sqrt(log( pow((sd_mean_sigma[isi_i]/mean_mean_sigma[isi_i]), 2) + 1 ))
    logmu_mean_sigma[isi_i] <- log(mean_mean_sigma[isi_i]) - pow(logsigma_mean_sigma[isi_i], 2) / 2 
    
    mean_mu[isi_i] ~ dnorm(mean_mean_mu[isi_i], 1/pow(sd_mean_mu[isi_i], 2)) 
    sd_mu[isi_i] ~ dt(0, 1/pow(median_sd_mu[isi_i], 2), 1) T(0, )
  }
}"
